...
psql -h $POSTGRESQL_DB_HOST -d panel -U $POSTGRESQL_DB_USERNAME -c 'delete from pu; delete from places;'
...
ruby scan_PU.rb $1 $2 -105.38 19.73 -100.38 18.73 1.0
ruby scan_PU.rb $1 $2 -100.38 19.73 -98.38 18.73 0.05
ruby scan_PU.rb $1 $2 -98.38 19.73 -95.38 18.73 1.0
...
psql -h $POSTGRESQL_DB_HOST -d panel -U $POSTGRESQL_DB_USERNAME -c 'update pu set city_id = (select id from cities where ST_Contains(geom, pu.position) limit 1) where city_id is null;'
psql -h $POSTGRESQL_DB_HOST -d panel -U $POSTGRESQL_DB_USERNAME -c 'refresh materialized view vw_pu;'
psql -h $POSTGRESQL_DB_HOST -d panel -U $POSTGRESQL_DB_USERNAME -c "update updates set updated_at = current_timestamp where object = 'pu';"
