...
db = PG::Connection.new(:hostaddr => ENV['POSTGRESQL_DB_HOST'], :dbname => 'ch_panel', :user => ENV['POSTGRESQL_DB_USERNAME'], :password => ENV['POSTGRESQL_DB_PASSWORD'])
...
db.prepare('insert_ur',"insert into ur (id,position,open,resolved_by,resolved_on,created_on,resolution,type) values ($1,ST_SetSRID(ST_Point($2, $3), 4326),$4,$5,$6,$7,$8,$9)")
    ...
    while latStart > latSouth do
      ...
      begin
        ...
        json['mapUpdateRequests']['objects'].each do |u|
          begin
            if not u['resolvedOn'].nil? or u['open']
              db.exec_prepared('insert_ur', [u['id'], u['geometry']['coordinates'][0], u['geometry']['coordinates'][1], u['open'], u['resolvedBy'], (u['resolvedOn'].nil? ? nil : Time.at(u['resolvedOn']/1000)), Time.at(u['driveDate']/1000), u['resolution'], u['type'] ] ) if db.exec_params('select id from ur where id = $1',[u['id']]).count == 0
#              urs_area << u['id']
